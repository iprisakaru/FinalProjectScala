...
trait FilmApi extends FilmJsonMapping with FilmService with DataJsonMapping {
  val filmRoute: Route = {
    post {
      entity(as[NewFilm]) { customer => {
        complete(createFilmWithoutFilling(customer).map(_.toJson))
      }
      }
  ...
  val filmHelpRoute: Route = {
    post {
      entity(as[NewFilm]) { customer => {
        complete(createFilmWithFilling(customer).map(_.toJson))
      }
      }
    }
  }
